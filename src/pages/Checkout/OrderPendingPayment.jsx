...
import { FiClock, FiCopy, FiBox, FiArrowRight, FiCreditCard, FiCheckCircle } from "react-icons/fi";
import { useState } from "react";
import PaymentSimulationModal from "src/components/PaymentSimulationModal";

export default function OrderPendingPayment() {
  ...
  return (
    <div className="min-h-screen pt-32 pb-20 px-4 md:px-8 flex items-center justify-center relative overflow-hidden text-(--font-color)">
      ...
      <div className="absolute top-1/4 left-1/2 -translate-x-1/2 w-[600px] h-[600px] bg-yellow-500/10 rounded-full blur-[120px] pointer-events-none z-0"></div>

      <div className="relative z-10 w-full max-w-2xl bg-[#24201d]/60 backdrop-blur-xl border border-yellow-500/20 rounded-[2.5rem] p-8 md:p-12 shadow-2xl text-center animate-in zoom-in-95 duration-700">
        ...
        <h1 className="text-4xl font-serif font-bold text-white mb-4">Zamówienie zarejestrowane</h1>
        <p className="text-lg opacity-70 mb-4 max-w-md mx-auto">
          Twoje zamówienie zostało zapisane, ale wymaga jeszcze opłacenia.
        </p>
        <p className="text-sm text-yellow-400/80 mb-10 max-w-sm mx-auto font-bold">
          Masz 24 godziny na dokonanie płatności, po tym czasie zamówienie zostanie automatycznie anulowane.
        </p>
        ...
        {trackingNumber && (
            <div className="bg-black/30 border border-white/5 rounded-3xl p-8 mb-10 relative overflow-hidden group">
                ...
                <p className="text-sm opacity-60 uppercase tracking-widest font-bold mb-2">Twój numer zamówienia</p>
        ...
        {/* Przycisk Opłać teraz */}
        <div className="bg-[#24201d] border border-green-500/30 rounded-3xl p-6 mb-10 shadow-[0_0_20px_rgba(34,197,94,0.15)]">
            <p className="text-sm opacity-80 mb-4 font-bold text-white">Opłać swoje zamówienie, abyśmy mogli je zrealizować:</p>
            <button 
                onClick={() => setSimulationOpen(true)}
                className="w-full sm:w-auto py-4 px-8 bg-green-500 hover:bg-green-600 text-white rounded-xl font-bold transition-all flex items-center justify-center gap-2 mx-auto shadow-lg hover:scale-105 cursor-pointer"
            >
                <FiCreditCard size={20} /> Opłać teraz
            </button>
        </div>
