...
function Sidebar({filters, isFilterMenuOpen, toggleFilterMenu}) {
  ...
  return (
    <>
      <div className={`${isMobile ? `fixed overflow-x-hidden overflow-y-auto w-full z-20 backdrop-blur-sm backdrop-brightness-60 h-full mx-2 p-4 bottom-0 text-sm ${isFilterMenuOpen ? 'bottom-0' : 'bottom-1000 transition-all duration-400 ease-in-out'}`: 'flex absolute pl-4 w-1/8 mt-5 flex-col'}`}>
        <section >
          <div className="not-lg:hidden z-4 relative mb-13 text-3xl">
              <h1><BsShop/></h1>
          </div>
